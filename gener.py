...
from dataset.utils import crop_back_to_original
try:
    ...
    def start_validation(self, is_train=False):
        ...
        for loader in data:
            for iter_idx, batch in enumerate(tqdm(loader)):
                ...
                samples_save_base_path = os.path.join(self.config.gen_path, "gen")
                imgs_save_base_path = os.path.join(self.config.gen_path, "real")

                os.makedirs(samples_save_base_path, exist_ok=True)
                os.makedirs(imgs_save_base_path, exist_ok=True)

                for i in range(b):
                    for j in range(n):
                        padded_samples = TF.to_pil_image(samples[i][j] * 0.5 + 0.5)
                        padded_imgs = TF.to_pil_image(batch["image"][i][j] * 0.5 + 0.5)

                        original_size = tuple(image_size[i][j])  # (height, width)
                        cropped_samples = crop_back_to_original(padded_samples, original_size, padded_samples.size[::-1])
                        cropped_imgs = crop_back_to_original(padded_imgs, original_size, padded_imgs.size[::-1])

                        samples_save_path = os.path.join(samples_save_base_path, save_image_path[j][i])
                        imgs_save_path = os.path.join(imgs_save_base_path, save_image_path[j][i])

                        cropped_samples.save(samples_save_path)
                        cropped_imgs.save(imgs_save_path)
