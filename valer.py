...
class Valer:
    def __init__(self, config):
        ...
        if config.sample_index > 0:
            val_indices = list(range(config.sample_index, config.sample_index + 1))
            print("sub_val_indices: ", val_indices)
    ...
    def start_validation(self, val_or_train=False):

        self.model.eval()
        if not val_or_train:
            data = [self.loader_val]
        # else:
        #     data = [self.loader_train]
            ...
            for iter_idx, batch in enumerate(tqdm(loader)):
                ...
                for i in range(b):
                    ...
                    for j in range(n):
                        ...
                        im = self.dataset_val.datasets[0].vis_getitem_data(out=temp_data, return_tensor=True, print_scene_description=False)
                ...
                for i in range(b):
                    images_per_cam = []
                    for j in range(n):
                        temp_data = {"image": samples[i][j], "box":batch["box"][i][j], "box_name":batch["box_name"][i][j]}
                        im = self.dataset_val.datasets[0].vis_getitem_data(out=temp_data, return_tensor=True, print_scene_description=False)
                        images_per_cam.append(im)
                    images_per_cam = torch.stack(images_per_cam)
                ...
                road_maps_with_box_drawing = None
                if "road_map" in batch:
                    road_maps_with_box_drawing = [] # we save this durining trianing for better visualization
                    for i in range(b):
                        images_per_cam = []
                        for j in range(n):
                            temp_data = {"image":batch["road_map"][i][j] , "box":batch["box"][i][j], "box_name":batch["box_name"][i][j]}
                            im = self.dataset_val.datasets[0].vis_getitem_data(out=temp_data, return_tensor=True, print_scene_description=False)
                            images_per_cam.append(im)
                        images_per_cam = torch.stack(images_per_cam)
                        road_maps_with_box_drawing.append(images_per_cam)
                    road_maps_with_box_drawing = torch.stack(road_maps_with_box_drawing)
                if road_maps_with_box_drawing != None:
                    self.image_caption_saver(images=sample_images_with_box_drawing, real=real_images_with_box_drawing, background=road_maps_with_box_drawing, seen=iter_name)
                else:
