...
def read_official_nusc_ckpt(ckpt_path):      
    "Read offical pretrained PerLDiff ckpt in Nuscenes" 
    print( "\n" + "*" * 20 + " load model from {}!".format(ckpt_path) + " *" * 20 + "\n")

    state_dict = torch.load(ckpt_path, map_location="cpu")

    for name in list(state_dict['model'].keys()):
        if ("transformer_blocks" in name) and ("cross_view_left" in name):
                # New added Attention layers 
            del  state_dict['model'][name]
...
class Trainer:
    def __init__(self, config):
        ...
        if config.nusc_ckpt_path != None:
            state_dict = read_official_nusc_ckpt(config.nusc_ckpt_path)
        ...
        if config.nusc_ckpt_path != None:    
            ...
            missing_keys, unexpected_keys = self.model.load_state_dict( official_state_dict["model"], strict=False  )
            assert unexpected_keys == []
            original_params_names = list( official_state_dict["model"].keys()  ) # used for sanity check later 
