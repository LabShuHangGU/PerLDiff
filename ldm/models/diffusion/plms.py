...
class PLMSSampler(object):
    ...
    @torch.no_grad()
    def sample(self, S, shape, input, guidance_scale_c=None, mask=None, x0=None):
        self.make_schedule(ddim_num_steps=S)
        return self.plms_sampling(shape, input, guidance_scale_c, mask=mask, x0=x0)


    @torch.no_grad()
    def plms_sampling(self, shape, input, guidance_scale_c=None, mask=None, x0=None):
        b, n, _, _, _ = shape
        ...
        # iterator = tqdm(time_range, desc='PLMS Sampler', total=self.total_steps)
        iterator = time_range
        for i, step in enumerate(iterator):
            ...
            img, pred_x0, e_t = self.p_sample_plms(input, ts, index=index, guidance_scale_c=guidance_scale_c, old_eps=old_eps, t_next=ts_next)
            input["x"] = img
    ...
    @torch.no_grad()
    def p_sample_plms(self, input, t, index, guidance_scale_c=None, old_eps=None, t_next=None):
        x = deepcopy(input["x"]) 
        b, n, _, _, _ = x.shape
        ...
        def get_model_output(input):
            ...
            if guidance_scale_c != None:
                unconditional_input = dict(x=input["x"], timesteps=input["timesteps"])
