...
class DDIMSampler(object):
    ...
    def sample(self, S, shape, input, guidance_scale_c=None, mask=None, x0=None):
        ...
        return self.ddim_sampling(shape, input, guidance_scale_c,  mask=mask, x0=x0)
 

    @torch.no_grad()
    def ddim_sampling(self, shape, input, guidance_scale_c=None, mask=None, x0=None):
        b, n, _, _, _ = shape
        ...
        # iterator = tqdm(time_range, desc='DDIM Sampler', total=self.total_steps)
        iterator = time_range
        ...
        for i, step in enumerate(iterator):
            ...
            img, pred_x0 = self.p_sample_ddim(input, index=index, guidance_scale_c=guidance_scale_c)
    ...
    @torch.no_grad()
    def p_sample_ddim(self, input, index, guidance_scale_c=None):
