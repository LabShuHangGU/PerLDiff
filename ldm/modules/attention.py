...
class CrossAttention(nn.Module):
    def __init__(self, query_dim, key_dim, value_dim, heads=8, dim_head=64, dropout=0, atten_map_res=[48, 160], max_boxes=25, max_length=77):
        super().__init__()
...
class GatedCrossAttentionDense(nn.Module):
    ...
    def forward(self, x, objs, perl_box_masking_map=None, perl_road_masking_map=None):
        ...
        x = x + self.scale*torch.tanh(self.alpha_attn) * self.attn( self.norm1(x), objs, objs, perl_box_masking_map=perl_box_masking_map, perl_road_masking_map=None) 
...
class BasicTransformerBlock(nn.Module):
    def __init__(self, query_dim, key_dim, value_dim, n_heads, d_head, fuser_type, use_checkpoint=True, num_camera=1):
        super().__init__()
    ...
    def forward(self, x, context, objs, perl_box_masking_map):
        if self.use_checkpoint and x.requires_grad:
            return checkpoint.checkpoint(self._forward, x, context, objs, perl_box_masking_map)
        else:
            return self._forward(x, context, objs, perl_box_masking_map)

    def _forward(self, x, context, objs, perl_box_masking_map): 
...
class SpatialTransformer(nn.Module):
    def __init__(self, in_channels, key_dim, value_dim, n_heads, d_head, depth=1, fuser_type=None, use_checkpoint=True, num_camera=1):
        super().__init__()
    ...
    def forward(self, x, context, objs, perl_box_masking_map=None):
        n =  self.num_camera
        ...
        for block in self.transformer_blocks:
            x = block(x, context, objs, perl_box_masking_map)
